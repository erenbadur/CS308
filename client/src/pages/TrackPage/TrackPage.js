  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get(`/api/track/track/${userId}`); // Fetch the latest order
        setOrders(response.data.order ? [response.data.order] : []); // Wrap the single order in an array
      } catch (err) {
